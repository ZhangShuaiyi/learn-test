...
fn main() {
    ...
    let mmio_resources = [Resource::MmioAddressRange {
        base: 0xc0000000,
        size: 0x2000,
    }];
    let foo_block = FooBlock::default();
    let foo_mmio = Arc::new(FooMmioDevice { device: foo_block });
    device_mgr
        .lock()
        .unwrap()
        .register_device_io(foo_mmio, &mmio_resources)
        .unwrap();
...
const MMIO_VERSION_2: u32 = 2;
const MMIO_CFG_SPACE_OFF: u64 = 0x100;

const REG_MMIO_MAGIC_VALUE: u64 = 0x000;
const REG_MMIO_VERSION: u64 = 0x004;
const REG_MMIO_DEVICE_ID: u64 = 0x008;
const REG_MMIO_VENDOR_ID: u64 = 0x00c;
const REG_MMIO_DEVICE_FEATURE: u64 = 0x010;
const REG_MMIO_DEVICE_FEATURES_S: u64 = 0x014;
const REG_MMIO_DRIVER_FEATURE: u64 = 0x020;
const REG_MMIO_DRIVER_FEATURES_S: u64 = 0x024;
// const REG_MMIO_GUEST_PAGE_SIZ: u64 = 0x028;
// const REG_MMIO_QUEUE_SEL: u64 = 0x030;
// const REG_MMIO_QUEUE_NUM_MA: u64 = 0x034;
// const REG_MMIO_QUEUE_NUM: u64 = 0x038;
// const REG_MMIO_QUEUE_ALIGN: u64 = 0x03c;
// const REG_MMIO_QUEUE_PFN: u64 = 0x040;
// const REG_MMIO_QUEUE_READY: u64 = 0x044;
// const REG_MMIO_QUEUE_NOTIF: u64 = 0x050;
// const REG_MMIO_INTERRUPT_STAT: u64 = 0x060;
// const REG_MMIO_INTERRUPT_AC: u64 = 0x064;
const REG_MMIO_STATUS: u64 = 0x070;
// const REG_MMIO_QUEUE_DESC_LOW: u64 = 0x080;
// const REG_MMIO_QUEUE_DESC_HIGH: u64 = 0x084;
// const REG_MMIO_QUEUE_AVAIL_LOW: u64 = 0x090;
// const REG_MMIO_QUEUE_AVAIL_HIGH: u64 = 0x094;
// const REG_MMIO_QUEUE_USED_LOW: u64 = 0x0a0;
// const REG_MMIO_QUEUE_USED_HIGH: u64 = 0x0a4;
...
impl FooBlock {
    fn default() -> Self {
        let mut avail_features = 1u64 << virtio_blk::VIRTIO_F_VERSION_1;
        avail_features |= 1u64 << virtio_blk::VIRTIO_BLK_F_SIZE_MAX;
        avail_features |= 1u64 << virtio_blk::VIRTIO_BLK_F_SEG_MAX;
        FooBlock {
    ...
    fn set_features_select(&mut self, v: u32) {
        self.features_select = v;
    }

    fn set_acked_features_select(&mut self, v: u32) {
        self.acked_features_select = v;
    }
}

struct FooMmioDevice {
    device: FooBlock,
}

impl DeviceIo for FooMmioDevice {
    fn read(&self, _base: IoAddress, offset: IoAddress, data: &mut [u8]) {
        let offset = offset.raw_value();

        if offset >= MMIO_CFG_SPACE_OFF {
            println!("MMIO_CFG_SPACE_OFF not support!!!!");
            return;
        }
        if data.len() != 4 {
            println!("mmio read len:0x{:x}", data.len());
            return;
        }
        let v = match offset {
            REG_MMIO_MAGIC_VALUE => MMIO_MAGIC_VALUE,
            REG_MMIO_VERSION => MMIO_VERSION_2,
            REG_MMIO_DEVICE_ID => self.device.device_type(),
            REG_MMIO_VENDOR_ID => 0xdbfcdb00,
            REG_MMIO_DEVICE_FEATURE => self.device_features(),
            _ => {
                println!("unknown virtio mmio readl at 0x{:x}", offset);
                return;
            }
        };
    ...
    fn write(&self, _base: IoAddress, offset: IoAddress, data: &[u8]) {
        let offset = offset.raw_value();
        if (MMIO_CFG_SPACE_OFF..0x1000).contains(&offset) {
            println!("write to update mmio config space!");
            return;
        }
        if data.len() != 4 {
            println!(
        ...
        match offset {
            REG_MMIO_DEVICE_FEATURES_S => self.device().set_features_select(v),
            REG_MMIO_DRIVER_FEATURE => self.set_acked_features(v),
            REG_MMIO_DRIVER_FEATURES_S => self.device().set_acked_features_select(v),
            REG_MMIO_STATUS => self.update_driver_status(v),
            _ => println!("unknown virtio mmio writel to 0x{:x}", offset),
        }
    }
...
impl FooMmioDevice {
    fn device(&self) -> FooBlock {
        return self.device;
    }

    fn device_features(&self) -> u32 {
        match self.device.features_select {
            0 => self.device.avail_features as u32,
            1 => (self.device.avail_features >> 32) as u32,
            _ => {
                println!("query features page: {}", self.device.features_select);
                0u32
            }
        }
    }
    ...
    fn set_acked_features(&self, v: u32) {
        ...
        let select = device.acked_features_select;
        let value = match select {
            0 => v as u64,
            1 => (v as u64) << 32,
            _ => {
                println!("ack unknown feature page: {}", select);
                0u64
            }
        };
        device.acked_features |= value;
    }

    fn update_driver_status(&self, v: u32) {
        println!("update_driver_status:0x{:x}", v);
    }
}
